...
' 08-05-2014       Abdul Jabbar       CR# 307  SMS functionality issues needs to be fix
...
Module ModUtlGlobalFunctions
    ...
    Public Function SendSMS(ByVal USErName As String, ByVal password As String, ByVal MessageText As String, ByVal number As String, ByVal CompanyName As String) As String
        Try
            ...
            Catch e As Exception
                'CR#307
                'Return e.Message
                If e.Message = "The remote name could not be resolved: 'www.sms4connect.com'" Then
                    Return "Internet connection problem, problem check internet connectivity and try again"
                    Exit Function
                Else
                    Return e.Message
                    Exit Function
    ...
    Public Sub FillApplicationHashTable()
        ...
        frmLogin.prgBar1.Value = frmLogin.prgBar1.Value + 1
        frmLogin.lblStatusBarComment.Text = "Loading Mushroom Common List"
        Application.DoEvents()
        ''Add Mushroom Common List in Hashtable
        AddMushroomCommonListToHashTable()
    ...
    End Sub
    Public Sub AddMushroomCommonListToHashTable()

        ''Remove existing object from hashtable
        If gObjMyAppHashTable.ContainsKey(EnumHashTableKeyConstants.GetMushroomCommonList.ToString()) Then gObjMyAppHashTable.Remove(EnumHashTableKeyConstants.GetMushroomCommonList.ToString())

        Dim objDal As New CBOCollectionDAL
        Dim dt As DataTable = objDal.GetMushroomCommonList()
        gObjMyAppHashTable.Add(EnumHashTableKeyConstants.GetMushroomCommonList.ToString(), dt)
    ...
    Public Function GetFormSecurityControls(ByVal strFromName As String) As NameValueCollection
        Try
            ...
            Dim dt As DataTable = New SecurityUserDAL().GetUserFormRights(gObjUserInfo.UserID)
            dt.TableName = "Rights"
    ...
    Public Sub ApplyStyleSheet(ByVal objCtrl As Control, ByVal FormID As String, Optional ByVal strCallingFrom As String = "Form")
        Try
            ...
            For Each ctl In objCtrl.Controls
                ...
                If ctl.HasChildren Then
                    ...
                    ApplyStyleSheet(ctl, FormID, IIf(strCallingFrom = "Form", "Form", "Report"))
                End If
                If TypeOf ctl Is System.Windows.Forms.TextBox Then
                    ctl.Font = gobjDefaultFontSettingForInput
                    If gEnumIsRightToLeft = RightToLeft.Yes Then
    ...
    Public Sub ShowProgressBar(Optional ByVal ProgressText As String = "")

        Application.DoEvents()

        frm = New frmProgressbar
        frm.ShowInTaskbar = False
        frm.TopLevel = True
        frm.TopMost = True
        frm.StartPosition = FormStartPosition.CenterScreen
        frm.Opacity = 90
        If Not ProgressText = String.Empty Then
            frm.ProgressText = ProgressText
        End If
        frm.Show()
    ...
    Public Sub EndProgressBar()
        frm.Close()
    End Sub
    'CR#300 ,function will be used to Read Companies against logged in user
    Public Function GetUserCompanies(ByVal UserID) As String
        Try
